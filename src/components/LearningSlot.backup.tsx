...
type JobRow = { num: string; kws: { label: string; match: boolean }[]; price: string };
const JOBS: JobRow[] = [
  { num: '19', kws: [
...
const LEARNING_SEGS: Seg[] = [
  ...
  { key: 'j1',    value: 'Offert #19 — element · sovrum · fukt — 41 500 kr' },
  { key: 'j2',    value: 'Offert #24 — element · sovrum — 49 800 kr' },
  { key: 'j3',    value: 'Offert #28 — element · sovrum · nyproduktion — 38 400 kr' },
  { key: 'j4',    value: 'Offert #31 — element · kök · ventilation — 28 000 kr' },
  ...
  { key: 'm3',    value: 'Kopplingsrör 15 mm — 2 av 4 jobb' },
  { key: 'm4',    value: 'Köksfläkt-anslutning 125 mm — 1 av 4 jobb — filtrerat från #31' },
          ...
          <div className="mt-1 space-y-[2px]">
            {JOBS.map((j, i) => shown(`j${i + 1}`) && (
              <SlideRow key={j.num}>
                ...
                >
                  <span className="font-mono text-stone-500">Offert&nbsp;#{j.num}</span>
                  <span className="flex items-center gap-1 min-w-0 flex-wrap">
                    {j.kws.map((k) => (
                      <MiniChip key={k.label} active={k.match}>{k.label}</MiniChip>
                    ))}
                  </span>
                  <span className="font-mono text-right tabular-nums text-stone-800">
                    {j.price}
                  </span>
                </div>
              </SlideRow>
            ...
            {MATS.map((m, i) => shown(`m${i + 1}`) && (
              <SlideRow key={m.name}>
                ...
                >
                  ...
                  <span className="font-mono tabular-nums text-right text-stone-600">
                ...
                {m.excluded && (
                  <div
                    className="text-right font-mono text-stone-400"
                    style={{ fontSize: '9.5px', paddingRight: '2px', marginTop: '-1px' }}
                  >
                    från&nbsp;{m.excluded.source}
                  </div>
                )}
              </SlideRow>
            ))}
